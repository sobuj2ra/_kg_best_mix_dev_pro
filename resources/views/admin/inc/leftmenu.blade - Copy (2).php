...
<li class="header" style="text-align: center;color: #F79F1F;letter-spacing: 1px;font-weight: bold;">
  ...
  $permission_level = DB::table('users')
          ->where('id', Auth::user()->id)
          ->first();

  $privilege =explode(",",$permission_level->menu_permitted);
  $mysqli = new mysqli('localhost', 'root', '', 'passtrack');
  for($i = 0; $i < count($privilege); $i++)

  {
    $parent_id =  $privilege[$i];

    $sql = "SELECT a.id, a.menu, a.url_link, a.parent_id, (SELECT count(*) FROM menus b WHERE a.id = b.parent_id) AS no_of_child FROM menus a WHERE id = $parent_id";
    // var_dump($sql);
    // exit();
    ...
    if ($result->num_rows > 0) {
      ...
      while ($role = $result->fetch_assoc())
      {
        if($role['parent_id'] == 0)
        {
          //echo "<li><a>".$role['menu_name']."</a>";
          echo "
            <li class='treeview'>
                <a href='#'><i class='fa fa-file'></i><span>".$role['menu']."</span>
        ...
        {
          //echo "<li><a>".$role['menu_name']."</a></li>";{{URL::to('$role[menu_url]')}}
          echo "<li><a href=".'{}'."><i class='fa fa-circle-o icon_tree_color'></i>".$role['menu']."</a></li>";
        }
...
//
//  $permission_sub = DB::table('menus')->distinct()->where('id', $permission_child->parent_id)->get();
//  var_dump($permission_sub);
//}
//  if ($permission_sub->parent_id == 0){
//    echo $permission_sub->menu;
//  }
